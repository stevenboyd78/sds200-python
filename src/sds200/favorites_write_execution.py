...
class _CopiedTreeEvidence:
    device: int
    inode: int
    sha256: str


class _CopiedTreeUnsafeError(RuntimeError):
    def __init__(
        self,
        path: Path,
        message: str,
...
def _favorites_storage_snapshot_sha256(
    snapshot: FavoritesStorageSnapshot,
) -> str:
    if not isinstance(
        snapshot,
