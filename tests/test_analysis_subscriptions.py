...
from sds200.analysis_subscriptions import (
    ANALYSIS_DEFAULT_QUEUE_CAPACITY,
    AnalysisPublisher,
    AnalysisSubscriptionClosed,
)
from sds200.models import AnalysisResponse
from sds200.xml_protocol import AnalysisParser


def response(identifier: str) -> AnalysisResponse:
    return AnalysisParser().parse(
        "AST", f'<AST><CurrentActivity Id="{identifier}" /></AST>'
    )
...
def test_overflow_drops_oldest_and_exposes_cumulative_loss() -> None:
    publisher = AnalysisPublisher(queue_capacity=2)
    subscription = publisher.subscribe()
...
def test_slow_subscriber_cannot_delay_or_drop_healthy_subscriber() -> None:
    publisher = AnalysisPublisher(queue_capacity=1)
...
def test_timeout_rejects_non_numeric_values(timeout: object) -> None:
    subscription = AnalysisPublisher().subscribe()
...
def test_timeout_expiration_raises_queue_empty() -> None:
    with pytest.raises(queue.Empty):
        AnalysisPublisher().subscribe().get(0)
...
def test_subscription_close_is_idempotent_and_removes_subscriber() -> None:
    ...
    with pytest.raises(AnalysisSubscriptionClosed):
        subscription.get(0)
...
def test_publisher_close_wakes_blocked_subscription() -> None:
    ...
    assert result == [AnalysisSubscriptionClosed]
    publisher.close()
...
def test_subscriber_limit() -> None:
    publisher = AnalysisPublisher(max_subscribers=1)
    publisher.subscribe()
...
def test_constructor_bounds_reject_non_integer_values(field: str, value: object) -> None:
    with pytest.raises(TypeError):
        AnalysisPublisher(**{field: value})  # type: ignore[arg-type]
...
def test_constructor_bounds_reject_values_below_one(field: str, value: int) -> None:
    with pytest.raises(ValueError):
        AnalysisPublisher(**{field: value})


def test_defaults_and_publish_type_validation() -> None:
    publisher = AnalysisPublisher()
    assert publisher.queue_capacity == ANALYSIS_DEFAULT_QUEUE_CAPACITY
...
def test_concurrent_publishers_preserve_global_sequence_order() -> None:
    publisher = AnalysisPublisher(queue_capacity=200)
