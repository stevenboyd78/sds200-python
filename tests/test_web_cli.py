...
def test_web_parser_accepts_explicit_local_options() -> None:
    ...
    assert args.daemon_socket_path == Path(
        "/tmp/sdsctl/daemon.sock"
    )
...
def test_web_parser_rejects_non_loopback_address(
    address: str,
) -> None:
...
def test_web_cli_builds_daemon_client_and_runs_server(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    FakeDaemonApiClient.instances.clear()
    captured_factories: list[Callable[[], object]] = []
    app = object()
    ...
    def fake_create_app(
        ...
        captured_factories.append(api_client_factory)
        return app
    ...
    def fake_run_server(
        ...
        *,
        ...
        server_calls.append(
            (selected_app, host, port, access_log)
        )
        return 0
    ...
    daemon_client = captured_factories[0]()
